...
request.get (url2, (error, response, body) => {
    ...
    for (var i = 0; i < s.length; i++) {
      for (var x = 0; x <= s[i].players.length; x++) {
        ...
        z++
        if (x > p.player_count - 2) {if (found === false) {return msg.channel.send("Player not found, either there in the lobbies or offline")}}
        console.log("Looking through: " + s[i].players)
